...
[3. 操作流程](#3-操作流程)
  ...
  - [3.2. 主畫面](#32-主畫面)
  - [3.3. 多網站管理](#33-多網站管理)
    - [3.3.1. 前台與後台對應呈現](#331-前台與後台對應呈現)
    - [3.3.2. 節點功能介紹](#332-節點功能介紹)
      - [3.3.2.1. 新增節點](#3321-新增節點)
      - [3.3.2.2. 修改節點](#3322-修改節點)
      - [3.3.2.3. 刪除節點](#3323-刪除節點)
      - [3.3.2.4. 移動節點](#3324-移動節點)
      - [3.3.2.5. 編輯頁面](#3325-編輯頁面)
      - [3.3.2.6. 預覽頁面](#3326-預覽頁面)
      - [3.3.2.7. 送審頁面](#3327-送審頁面)
    - [3.3.3. 編輯器模組](#333-編輯器模組)
      - [3.3.3.1. 版型編輯器模組](#3331-版型編輯器模組)
      - [3.3.3.2. 文稿編輯器模組](#3332-文稿編輯器模組)
  - [3.4. 我的申請單](#34-我的申請單)
  - [3.5. 審核、放行管理](#35-審核放行管理)
  - [3.6. 網站管理](#36-網站管理)
  - [3.7. 報表管理](#37-報表管理)
    - [3.7.1. 權限報表](#371-權限報表)
...
## **1. 簡介**
  ...
  Garden CMS 是由新逸資訊科技自主性研發之內容管理系統的產品，主要用內容管理系統（CMS）來維護各網站之內容，目前已成功導入多家金融、壽險等產業，使用者可透過本產品內建相關版型模組進行網頁的自主性維護，並且可搭配Garden CMS的設計規範來擴充相關更新穎的網頁版型，藉此讓後續維護人員可無縫銜接開發相關規範，以確保網站呈現的一致性。
...
- ### **3.2. 主畫面**
    ...
    |登出|登出，會重新回到登入頁|
    |目錄|此使用者能夠閱讀/操作的所有功能|
    |詳細頁面|點擊左側目錄項目（此處以多網站管理為範例）後，顯示對應的詳細頁面|
...
- #### **3.3. 多網站管理**
  ...
  1. 點擊官網管理（1） ➞ 多網站管理（2） ➞ 選擇網站列表（3）
      ...
      |欄位|說明|
      |--|--|
      |網站列表（3）|用 Garden CMS 管理的所有網站|
      |網站設定（4）|選擇左側網站列表後，會在右側顯示網站設定頁面|

      <br/>

  2. 選取官網的網站，進行《節點》[^1] 編輯
  ...
  3. 設定節點頁面
      ...
      <div class="text-center">【圖片】設定節點的細部功能畫面</div>

      ◎ 欄位說明

      |欄位|說明|
      |--|--|
      |網站節點結構|瀏覽選取的網站節點結構（如：官網的節點結構）|
      |節點詳細資料|左側選取節點後，右側顯示選取該節點詳細資料（如：主選單節點）|

    <br/>

  - #### **3.3.1. 前台與後台對應呈現**

      後台呈現與前台對應畫面是一比一對應，若要調整前台節點順序，僅須在後台調整排列順序即可
  ...
  - #### **3.3.2. 節點功能介紹**
    ...
    - #### **3.3.2.1. 新增節點**
      ...
      #### 2. 點擊新增後，會出現新增節點頁面
        ...
        |輸入欄位|是否必填|類型|說明|
        |--|--|--|--|
        |顯示在主選單|否|單選框|若勾選此欄位，代表此節點會顯示在主選單|
        |限 pc 可見|否|單選框|若勾選此欄位，代表 pc 瀏覽器環境下，能夠在正式網頁觀看此節點內容|
        |限 mobile 可見|否|單選框|若勾選此欄位，代表 mobile 瀏覽器環境下，能夠在正式網頁觀看此節點內容（若"限 pc 可見"及"限 mobile 可見"皆無勾選，代表無限制，pc 及 mobile 瀏覽器環境下都可以觀看此節點內容）|
    ...
    - #### **3.3.2.2. 修改節點**
      ...
      2. 出現編輯節點頁面
            ...
            欄位說明請參閱 [新增節點](#2-點擊新增後會出現新增節點頁面)，僅差別在

            1. 節點編輯不能改節點類型

            2. 節點編輯可以一次改多個語系內容的頁面，當一個節點新增好後，其他不同語系的節點也會共用對應資料（如：編輯中文版後，英文&日文也會同時共用資料）
    ...
    - #### **3.3.2.4. 移動節點**
      ...
      2. 請確認是否要移動此節點
          ...
          |圖示|功能欄位|說明|
          |--|--|--|
          |<img src="./assets/image/instruction/addAccountPopupCancel.png" style="width: 70px">|取消|取消此次節點移動位置並關閉 popup 頁面|
          |<img src="./assets/image/instruction/confirm.png" style="width: 70px">|確認|確認此次節點移動位置並關閉 popup 頁面|
      ...
      - **編輯頁面的內容**
        - **新增《版型》[^2]**
          ...
          #### 2. 新增版面(點擊綠色+後，會在左邊展開"新增版面"功能面版)
            ...
            關於版型編輯器細部操作，請參閱 [3.3.3.1. 版型編輯器模組](#3331-版型編輯器模組)
        ...
        - **編輯版型**
          ...
          6. 預覽頁面(詳細步驟請參閱 [3.3.2.6. 預覽頁面](#3326-預覽頁面))
