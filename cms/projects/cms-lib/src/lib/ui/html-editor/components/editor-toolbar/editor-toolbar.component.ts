...
import { tap, map, takeUntil, finalize, concatAll } from 'rxjs/operators';
...
export class EditorToolbarComponent implements OnInit {
  ...
  doAction(action: IHtmlEditorAction) {
    ...
    const shoulRestoreTarget =
      SHOULD_RESTORE_TAGS.indexOf((this.context.commonAncestorContainer as HTMLElement).tagName) > -1
        ? this.context.commonAncestorContainer
        : undefined;
    console.log('doAction() shoulRestoreTarget = ', shoulRestoreTarget);

    action.do().subscribe(_ => {
      const canRestore = shoulRestoreTarget && this.context.editorContainer.contains(shoulRestoreTarget) || false;
      console.log('doAction() canRestore = ', canRestore);
      if (canRestore) {
        this.context.simpleWysiwygService.setSelectionOnNode(shoulRestoreTarget);
      }
    });
  }
  ...
  fontStyle(style: string) {
    if (style == 'insertTable') {
      // try to add table
      var range = this.context.simpleWysiwygService.getRange();
